...
import { prepareReflectCandidateReconciliation, reconcileReflectHighlightsInMemory } from "./ReflectReconciliation";
  ...
  private async fetchReadwise(): Promise<void> {
    try {
      await fetchReconcileAndRender(
        () => this.deps.readwiseApi.fetchHighlights(async (candidate) => prepareReflectCandidateReconciliation(candidate.highlights, this.deps.reflectNotes, this.confirmedReflectFiles, (message) => new Notice(message))),
        () => Promise.resolve(),
        () => this.render()
      );
    } catch (error) {
