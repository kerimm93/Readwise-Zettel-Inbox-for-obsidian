...
import { buildReflectCandidateIndex, ReflectCandidate, ReflectCandidateIndex, ReflectFileResolution, resolveReflectFromIndex } from "./ReflectResolution";
...
export class ReflectNoteService {
  ...
  async buildIndex(): Promise<ReflectCandidateIndex<TFile>> {
    return buildReflectCandidateIndex(this.app.vault.getMarkdownFiles(), (file) => this.readCandidate(file));
  }

  resolveFromIndex(index: ReflectCandidateIndex<TFile>, highlightId: string): ReflectFileResolution<TFile> {
