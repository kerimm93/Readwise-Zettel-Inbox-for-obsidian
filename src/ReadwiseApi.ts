...
export class ReadwiseApi {
  ...
  async fetchHighlights(reconcileCandidate?: (state: InboxState) => Promise<void | (() => void)>): Promise<ReadwiseFetchResult> {
    ...
    const afterCommit = reconcileCandidate ? await reconcileCandidate(replacement.state) : undefined;
