...
export async function reconcileReflectHighlightsInMemory(highlights: Highlight[], reflectNotes: ReflectReconciliationNotes, confirmed: Set<string>, notify: (message: string) => void = () => {}, now: () => string = () => new Date().toISOString()): Promise<number> {
  confirmed.clear();
...
export async function prepareReflectCandidateReconciliation(highlights: Highlight[], reflectNotes: ReflectReconciliationNotes, visibleConfirmed: Set<string>, notify: (message: string) => void = () => {}, now: () => string = () => new Date().toISOString()): Promise<() => void> {
  ...
  await reconcileReflectHighlightsInMemory(highlights, reflectNotes, nextConfirmed, notify, now);
  return () => {
