...
var InboxView = class extends import_obsidian2.ItemView {
  ...
  async onOpen() {
    ...
    for (const highlight of this.deps.stateStore.getState().highlights.filter((item) => hasRoute(item, "reflect"))) {
      try {
        const resolution = await this.deps.reflectNotes.resolve(highlight);
...
function classifyReflectCandidates(candidates, highlightId) {
...
var ReflectNoteService = class {
  ...
  async resolve(highlight) {
    const cachedPath = (0, import_obsidian5.normalizePath)(highlight.workflow.reflectFilePath || "");
