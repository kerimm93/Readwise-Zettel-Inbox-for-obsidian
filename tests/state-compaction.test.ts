...
const throwingReflectNotes = {
  async buildIndex() { return { candidatesByHighlightId: new Map(), unreliableCandidates: [] }; },
  resolveFromIndex(_index: never, id: string) {
    if (id === "throws") throw new Error("boom");
    return { kind: "found" as const, file: { path: `Reflect/${id}.md` }, reflected: false };
  }
};
const strictState = buildActiveStateReplacement(previous, [h("throws", ["reflect"]), h("after-throw", ["reflect"])]);
const strictConfirmed = new Set(["old-confirmed"]);
let strictAfterCommit: (() => void) | undefined;
await assert.rejects(async () => {
  strictAfterCommit = await prepareReflectCandidateReconciliation(strictState.state.highlights, throwingReflectNotes as never, strictConfirmed);
}, /boom/);
assert.equal(strictAfterCommit, undefined);
assert.deepEqual([...strictConfirmed], ["old-confirmed"]);
let strictWrites = 0;
await assert.rejects(async () => {
  const after = await prepareReflectCandidateReconciliation(strictState.state.highlights, throwingReflectNotes as never, strictConfirmed);
  await writeThenSwapState(previous, strictState.state, async () => { strictWrites++; });
  after();
}, /boom/);
assert.equal(strictWrites, 0);
assert.deepEqual([...strictConfirmed], ["old-confirmed"]);

const bestEffortState = buildActiveStateReplacement(previous, [h("throws", ["reflect"]), h("after-throw", ["reflect"])]);
const notices: string[] = [];
const bestEffortConfirmed = new Set<string>();
const bestEffortChanged = await reconcileReflectHighlightsInMemory(bestEffortState.state.highlights, throwingReflectNotes as never, bestEffortConfirmed, (message) => notices.push(message), () => "best-effort-now");
assert.deepEqual(notices, ["boom"]);
assert.equal(bestEffortChanged, 1);
assert.deepEqual([...bestEffortConfirmed], ["after-throw"]);
assert.equal(bestEffortState.state.highlights.find((item) => item.id === "after-throw")!.workflow.reflectFilePath, "Reflect/after-throw.md");

const safeStrictReflectNotes = {
  async buildIndex() { return { candidatesByHighlightId: new Map(), unreliableCandidates: [] }; },
  resolveFromIndex(_index: never, id: string) {
    if (id === "ambiguous") return { kind: "ambiguous" as const };
    return { kind: "unresolved" as const };
  }
};
const safeStrictState = buildActiveStateReplacement(previous, [h("ambiguous", ["reflect"]), h("unresolved", ["reflect"])]);
const safeStrictConfirmed = new Set(["old-confirmed"]);
const safeAfterCommit = await prepareReflectCandidateReconciliation(safeStrictState.state.highlights, safeStrictReflectNotes as never, safeStrictConfirmed);
safeAfterCommit();
assert.deepEqual([...safeStrictConfirmed], []);
assert.equal(safeStrictState.state.highlights.every((item) => item.workflow.reflect === "open" && item.workflow.reflectFilePath === ""), true);
