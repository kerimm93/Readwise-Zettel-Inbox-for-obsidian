...
assert.deepEqual(reflectActionState(processedReflect, true), { canOpen: true, canReset: true, canMark: false, canSkip: false });
const resetReflect = withWorkflowStatus(processedReflect, "reflect", "open");
assert.equal(isWorkflowVisible(resetReflect), true);
assert.deepEqual(reflectActionState(resetReflect, true), { canOpen: true, canReset: false, canMark: true, canSkip: true });
...
const openReflect = visibility(["reflect"], "processed", "open");
const skippedReflect = visibility(["reflect"], "processed", "skipped");
const noReflect = visibility(["nothing"], "processed", "open");
const nothingReflect = visibility(["nothing", "reflect"], "processed", "open");
for (const [item, confirmed, expected] of [
  [openReflect, false, { canOpen: true, canReset: false, canMark: false, canSkip: true }],
  [openReflect, true, { canOpen: true, canReset: false, canMark: true, canSkip: true }],
  [processedReflect, true, { canOpen: true, canReset: true, canMark: false, canSkip: false }],
  [processedReflect, false, { canOpen: true, canReset: false, canMark: false, canSkip: false }],
  [skippedReflect, true, { canOpen: false, canReset: false, canMark: false, canSkip: false }],
  [noReflect, true, { canOpen: false, canReset: false, canMark: false, canSkip: false }],
  [nothingReflect, false, { canOpen: true, canReset: false, canMark: false, canSkip: true }],
  [nothingReflect, true, { canOpen: true, canReset: false, canMark: true, canSkip: true }]
] as const) assert.deepEqual(reflectActionState(item, confirmed), expected);
assert.deepEqual(reflectButtonLabels(reflectActionState(openReflect, false)), ["Reflect öffnen", "Reflect skip"]);
assert.deepEqual(reflectButtonLabels(reflectActionState(openReflect, true)), ["Reflect öffnen", "Als reflektiert markieren", "Reflect skip"]);
assert.deepEqual(reflectButtonLabels(reflectActionState(processedReflect, true)), ["Reflect öffnen", "Reflexion zurücksetzen"]);
assert.equal(reflectButtonLabels(reflectActionState(processedReflect, false)).some((label) => label.includes("reflektiert markieren")), false);

for (const [kind, expected] of [["found", true], ["missing", false], ["ambiguous", false], ["unresolved", false]] as const) assert.equal(isConfirmedReflectResolution(kind), expected);
const confirmedFiles = new Set<string>();
// Both successful openOrCreate and setReflected use this before their next render.
recordConfirmedReflectFile(confirmedFiles, "created");
recordConfirmedReflectFile(confirmedFiles, "reset");
assert.equal(confirmedFiles.has("created"), true);
assert.equal(confirmedFiles.has("reset"), true);
assert.equal(reflectActionState(withWorkflowStatus(openReflect, "reflect", "open"), confirmedFiles.has("reset")).canMark, true);

const order: string[] = [];
await fetchReconcileAndRender(
  async () => { order.push("fetch"); },
  async () => { order.push("reconcile"); },
  () => { order.push("render"); }
);
assert.deepEqual(order, ["fetch", "reconcile", "render"]);
const failedOrder: string[] = [];
await assert.rejects(fetchReconcileAndRender(
  async () => { failedOrder.push("fetch"); throw new Error("fetch failed"); },
  async () => { failedOrder.push("reconcile"); },
  () => { failedOrder.push("render"); }
), /fetch failed/);
assert.deepEqual(failedOrder, ["fetch"]);
const handledOrder: string[] = [];
await fetchReconcileAndRender(
  async () => { handledOrder.push("fetch"); },
  async () => { handledOrder.push("reconcile"); try { throw new Error("one highlight"); } catch { handledOrder.push("handled"); } },
  () => { handledOrder.push("render"); }
);
assert.deepEqual(handledOrder, ["fetch", "reconcile", "handled", "render"]);

// A newly fetched reflect tag is reconciled before its first post-fetch render.
const newlyTagged = visibility(["reflect"], "processed", "open");
const postFetchConfirmed = new Set<string>();
recordConfirmedReflectFile(postFetchConfirmed, newlyTagged.id);
const reflectedAfterFetch = withWorkflowStatus(newlyTagged, "reflect", reconcileReflectStatus(newlyTagged.workflow.reflect, true, true));
assert.deepEqual(reflectButtonLabels(reflectActionState(reflectedAfterFetch, postFetchConfirmed.has(newlyTagged.id))), ["Reflect öffnen", "Reflexion zurücksetzen"]);
const openAfterFetch = withWorkflowStatus(newlyTagged, "reflect", reconcileReflectStatus(newlyTagged.workflow.reflect, true, false));
assert.deepEqual(reflectButtonLabels(reflectActionState(openAfterFetch, true)), ["Reflect öffnen", "Als reflektiert markieren", "Reflect skip"]);
assert.deepEqual(reflectButtonLabels(reflectActionState(newlyTagged, false)), ["Reflect öffnen", "Reflect skip"]);
postFetchConfirmed.clear();
assert.equal(postFetchConfirmed.has(newlyTagged.id), false);
